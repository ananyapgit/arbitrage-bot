...
DISCORD_WEBHOOK_URL = "YOUR_DISCORD_WEBHOOK_URL_HERE" # Placeholder

POST_INTERVAL_SECONDS = 900  # 15 minutes
ANTI_SPAM_DELAY = 5
MAX_DEALS_PER_BATCH = 10  # Throttle: Max deals to process per run
MIN_DISCOUNT_THRESHOLD = 5.0 # Skip deals below this % unless high engagement
DRY_RUN = False  # set True to test without posting
TEST_MODE = False  # set True to ignore cache and not save updates
...
def add_affiliate_tag(url: str, marketplace: str) -> str:
    """
...
async def deal_engine():
    logging.info("🚀 Crawl.io automation started (Phase 5 Expansion)")
    ...
    while True:
        try:
            ...
            for deal in deals_to_process:
                ...
                original_url = url
                deal["url"] = add_affiliate_tag(url, marketplace)
            ...
            logging.info(f"Batch complete. Sent: {messages_sent}, Skipped(Cache): {skipped_cache_count}, LowDisc: {skipped_low_discount}, Invalid: {invalid_url_count}, EnrichFail: {enrichment_fail_count}, Tagged: {affiliate_tagged_count}, Throttled: {throttled_count}, RefSims: {referral_simulations}")
